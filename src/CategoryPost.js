...
class CategoryPost extends React.Component {
  ...
  fetchPosts(page,category) {
        ...
        butter.post.search(category).then((resp) => {
          let categoryPost = resp.data;
          //console.log(categoryPost);
          categoryPost.data.map((item,key)=>{
              let matchPost = 0;
              item.categories.map((cat,catKey)=>{
                  if(cat.name = category) matchPost++;
              });
              if(matchPost == 0){
                  //console.log(item);
                  //selectPost.push(item);
                  //categoryPost.data.splice(key,0);
                  delete categoryPost.data[key];
              }
            //console.log(`category item`, categoryPost);    
          });
          this.setState({
            loaded: true,
            resp: categoryPost
  ...
  componentWillMount() {
    ...
    let category = this.props.match.params.category || null;
    
    this.fetchPosts(page,category);
  }
                    ...
                    <Card className={classes.card}>
                        ...
                        <CardContent>
                            {post.categories.map((cat,key) => {
                                ...
                                if(!checkDuplicate)
                                return <Link to={`/category/${cat.name}`} key={`category-${key}`}><Chip color="primary" variant="outlined" label={cat.name} onClick={this.category} className={classes.chip} /></Link>
                            })}
                            </CardContent>
                        </CardActions>
                        <CardActions className={classes.actions} disableActionSpacing>
...
export default withStyles(styles)(CategoryPost);